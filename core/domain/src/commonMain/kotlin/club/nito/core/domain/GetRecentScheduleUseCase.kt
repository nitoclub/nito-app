...
sealed interface GetRecentScheduleUseCase {
    operator fun invoke(): Flow<FetchSingleContentResult<Schedule>>
}

class GetRecentScheduleExecutor(
    private val scheduleRepository: ScheduleRepository,
) : GetRecentScheduleUseCase {
    override fun invoke(): Flow<FetchSingleContentResult<Schedule>> = flow {
        val data = try {
        ...
        emit(FetchSingleContentResult.Success(data.first()))
    }
}
